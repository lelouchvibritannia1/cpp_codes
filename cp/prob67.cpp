...
void solve(){
    int n,k;
    cin>>n>>k;
    if(k==0) {cout<<"YES"<<endl;return;}
    sieve();
    int till=lower_bound(primesv.begin(),primesv.end(),n) - primesv.begin();
    for(int i=0;i<till;i++){
        if(primes.find(primesv[i]+primesv[i+1]+1)!=primes.end() && primesv[i]+primesv[i+1]+1<=n){
            k--;
            if(k==0) break;
        }
    }
